...
function render_list() {
  ...
  for (let el of el_recipes) {
    let recipe_categories = el.dataset.categories ? JSON.parse(el.dataset.categories) : [];
    let is_in = true;
    ...
    if (state.search) {
      ...
      let ingredients = el.dataset.ingredients ? JSON.parse(el.dataset.ingredients) : [];
      name_match = recipe_name.toLowerCase().includes(state.search.toLowerCase());
      let matched_ingredient = ingredients.find(
        (ingredient) => ingredient.toLowerCase().includes(state.search.toLowerCase())
      );
